...
def _aggregate_usage_rows(raw_rows: list) -> dict:
    """Aggregate raw DB rows by (profile, session_id, model, task) key.

    Sibling rows sharing the same key within a profile are SUMMED before any
    delta computation — prevents cursor oscillation between them (two rows for
    the same key, e.g. a main-usage row and a reasoning-tokens row, would
    otherwise flip the cursor on each sync).  Skips rows with an unparseable
    last_seen.  Returns {key: aggregated-dict}.
    """
    agg = {}
    for row in raw_rows:
...
def _collect_delta_row(key: str, row: dict, cursor: dict,
                       cfg: dict, anchor: Optional[float]):
    """Compute the positive delta since the cursor for one aggregated key.

    Advances ``cursor[key]`` to the current totals (realigning the cursor even
    when the delta is zero or negative) and returns the ledger row to append,
    or None when there is no positive delta.  Negative deltas (DB reset/rewind)
    are clamped to zero so the ledger never contains negative consumption.
    """
    ts = row["ts"]
...
def _append_ledger_rows(out: list, hermes_home: Optional[str] = None) -> None:
    """Append each row as one compact JSON line to the ledger file."""
...
def sync_ledger(hermes_home: Optional[str] = None,
    ...
    cursor = _load_json(cursor_path(hermes_home), {})
    appended = 0
    out = []
    ...
    agg = _aggregate_usage_rows(raw_rows)

    for key, row in agg.items():
        ledger_row = _collect_delta_row(key, row, cursor, cfg, anchor)
        if ledger_row:
            out.append(ledger_row)
            appended += 1
    if out:
        _append_ledger_rows(out, hermes_home)
    _save_json(cursor_path(hermes_home), cursor)
    return appended
