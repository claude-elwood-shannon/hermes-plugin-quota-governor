...
# Heartbeat file for cron-health-check.sh: obs-serve is a quiet daemon (its
# own log only grows on respawn), so log-mtime is NOT a liveness signal —
# the wrapper touches this file every tick that finds the portal healthy.
HEARTBEAT="$HERMES_HOME/logs/obs-serve.heartbeat"
