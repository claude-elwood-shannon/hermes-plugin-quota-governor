...
# ---------------------------------------------------------------------------
# GPU health — ml-host (192.168.1.32) via SSH + HTTP
# ---------------------------------------------------------------------------
...
def _ssh_gpu(cmd: str, timeout: int = 10) -> str:
    """Run a command on the GPU host via SSH; return stdout or ''."""
    try:
        r = subprocess.run(
...
def _http_json(url: str, timeout: int = 5) -> dict:
    """Fetch JSON from an HTTP endpoint using urllib (stdlib only)."""
...
def _gpu_cache_path(hermes_home=None) -> Path:
    return ms.state_dir(hermes_home) / "obs" / _GPU_CACHE


def _read_gpu_cache(hermes_home=None) -> dict | None:
    path = _gpu_cache_path(hermes_home)
    try:
...
def _write_gpu_cache(data: dict, hermes_home=None) -> None:
    path = _gpu_cache_path(hermes_home)
...
def read_gpu_health(hermes_home=None, now=None) -> dict:
    """Read GPU health from ml-host. Caches for _GPU_CACHE_TTL seconds.

    Returns dict with keys:
      temp_c, mem_used_mib, mem_total_mib, mem_pct, util_pct,
      model_id, model_ctx_len, vllm_active,
      rounds_total, rounds_today, rounds_today_ok, rounds_today_err,
      last_round, recent_rounds (list of last 10),
      source ('live' or 'cache'), ts
    """
    now = time.time() if now is None else float(now)

    # Check cache freshness
    cached = _read_gpu_cache(hermes_home)
    if cached and (now - cached.get("ts", 0)) < _GPU_CACHE_TTL:
        cached["source"] = "cache"
        return cached

    data: dict = {"ts": now, "source": "live"}

    # 1. nvidia-smi — temp, memory, utilization
    smi = _ssh_gpu(
    ...
    if smi:
        ...
        if len(parts) >= 4:
            ...
            data["mem_used_mib"] = int(parts[1])
            data["mem_total_mib"] = int(parts[2])
            data["util_pct"] = int(parts[3])
            mt = data["mem_total_mib"] or 1
            data["mem_pct"] = round(data["mem_used_mib"] / mt * 100.0, 1)

    # 2. vLLM active model via HTTP API
    models = _http_json(f"{_GPU_API}/v1/models")
    if models and models.get("data"):
        m = models["data"][0]
        data["model_id"] = m.get("id", "?")
        data["model_ctx_len"] = m.get("max_model_len")
        data["vllm_active"] = True
    else:
        data["vllm_active"] = False

    # 3. rounds.jsonl — execution history
    rounds_raw = _ssh_gpu("cat /data/ml/data/hermes/logs/rounds.jsonl", timeout=15)
    if rounds_raw:
        rounds = []
        for line in rounds_raw.strip().split("\n"):
            line = line.strip()
            if not line:
                continue
            try:
        ...
        # Today's rounds
        today_start = dt.datetime.now(CEST).replace(
            hour=0, minute=0, second=0, microsecond=0)
        today_epoch = today_start.timestamp()
        today_rounds = [r for r in rounds if r.get("ts", 0) >= today_epoch]
        data["rounds_today"] = len(today_rounds)
        data["rounds_today_ok"] = sum(
            1 for r in today_rounds if r.get("status") == "ok")
        data["rounds_today_err"] = sum(
            1 for r in today_rounds if r.get("status") != "ok")
        data["recent_rounds"] = rounds[-10:]
        data["last_round"] = rounds[-1] if rounds else None
        # Thermal history from recent rounds (for sparkline)
        data["temp_history"] = [
            (r.get("ts"), r.get("temp")) for r in rounds[-20:]
            if r.get("temp") is not None]
    else:
        data["rounds_total"] = 0
        data["recent_rounds"] = []

    # 4. vLLM service status
    svc = _ssh_gpu("systemctl --user is-active vllm 2>/dev/null", timeout=8)
    data["vllm_service"] = svc if svc else "unknown"

    # 5. vllm-rounds timer status
    timer = _ssh_gpu("systemctl --user is-active vllm-rounds.timer 2>/dev/null",
                     timeout=8)
    data["rounds_timer"] = timer if timer else "unknown"

    _write_gpu_cache(data, hermes_home)
    return data
...
def _shell(page: str, body: str, gen: str, sub: str = "") -> str:
    ...
    return "\n".join([
        ...
        "<code>kanban.db</code> · <code>weekly-reset-ledger.jsonl</code> · "
        "<code>gpu-health.json</code> (SSH ml-host)<br>",
...
def page_gpu(data: dict, query: dict = None) -> str:
    gpu = data.get("gpu", {})
    out: list[str] = []
    ...
    if last:
        out.append('<section class="card"><h2>Última ronda — detalle</h2>')
        out.append(f'<pre class="doc">{_esc(json.dumps(last, indent=2))}</pre>')
        out.append("</section>")
...
_PAGE_FN = {
    ...
    "providers": page_providers, "alarms": page_alarms, "gpu": page_gpu,
    "docs": page_docs,
}
