...
spec = importlib.util.spec_from_file_location(
    'trace_backfill',
    '/data/git/hermes-plugin-quota-governor/scripts/obs/trace-backfill.py')
trace_module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(trace_module)
tr = trace_module.tr
module = trace_module  # tests below reference helpers as module.<fn>
...
def test_run_backfill_dry_run(empty_trace, hermes_home):
    # create a cron/usage_audit.jsonl row in collect_usage_audit's shape
    # (fire_id + prompt/completion tokens + ts), not a trace-row shape.
    usage = [{
        "fire_id": "fire-1", "job_id": "job-1",
        "prompt_tokens": 10, "completion_tokens": 10,
        "model": "unknown-model", "ts": 10,
    }]
    ledger = [
    ...
    write_jsonl(usage, home / 'cron' / 'usage_audit.jsonl')
...
def test_run_backfill_exec(tmp_path):
    ...
    # create cron/usage_audit.jsonl (collector's real path + row shape)
    usage = [{
        "fire_id": "fire-1", "job_id": "job-1",
        "prompt_tokens": 10, "completion_tokens": 10,
        "model": "unknown-model", "ts": 10,
    ...
    assert report['ok'] and report['appended'] > 0
